mod tests {
    use rusp_lib::usp::Msg;
    use rusp_lib::usp_record::Record;

    #[test]
    fn simple_notify() {
        let bytes: Vec<u8> = vec![
            ,
        ];
        let payload_start = 78;
        let payload_expected = &bytes[payload_start..];

        let mut record = Record::from_bytes(&bytes).expect("Invalid Record bytes");
        let serialized = serde_json::to_value(&record).unwrap();

        let expected = serde_json::json!({
            "version": "1.0",
            "to_id": "proto::ax-usp-controller-nossl",
            "from_id": "proto::ax-usp-agent-nossl-websocket",
            "payload_security": "PLAINTEXT",
            "mac_signature": [],
            "sender_cert": [],
            "no_session_context": {
                "payload": payload_expected,
            }
        });
        assert_eq!(serialized, expected);

        let payload = record.payload_flatten().unwrap();
        assert_eq!(payload_expected, payload.as_slice());

        let msg = Msg::from_bytes(&payload).expect("Invalid Msg bytes");
        let serialized_msg = serde_json::to_value(&msg).unwrap();

        let expected_msg = serde_json::json!({
            "Header": {
              "msg_id": "test",
              "msg_type": "NOTIFY"
            },
            "Body": {
              "Request": {
                "Notify": {
                  "subscription_id": "subscription_id",
                  "send_resp": false,
                  "on_board_req": {
                    "oui": "oui",
                    "product_class": "product_class",
                    "serial_number": "serial_number",
                    "agent_supported_protocol_versions": "1.0"
                  }
                }
              }
            }
        });
        assert_eq!(serialized_msg, expected_msg);
    }

    #[test]
    fn create_obj() {
        let bytes: Vec<u8> = vec![
            ,
        ];
        let payload_start = 78;
        let payload_expected = &bytes[payload_start..];

        let mut record = Record::from_bytes(&bytes).expect("Invalid Record bytes");
        let serialized_record = serde_json::to_value(&record).unwrap();

        let expected = serde_json::json!({
            "version": "1.0",
            "to_id": "proto::ax-usp-agent-nossl-websocket",
            "from_id": "proto::ax-usp-controller-nossl",
            "payload_security": "PLAINTEXT",
            "mac_signature": [],
            "sender_cert": [],
            "no_session_context": {
                "payload": payload_expected,
            }
        });
        assert_eq!(serialized_record, expected);

        let payload = record.payload_flatten().unwrap();
        assert_eq!(payload_expected, payload.as_slice());

        let msg = Msg::from_bytes(&payload).expect("Invalid Msg bytes");
        let serialized_msg = serde_json::to_value(&msg).unwrap();

        let expected_msg = serde_json::json!({
            "Header": {
              "msg_id": "AXSS-1544114083.761508",
              "msg_type": "ADD"
            },
            "Body": {
              "Request": {
                "Add": {
                  "allow_partial": true,
                  "create_objs": [
                    {
                      "obj_path": "Device.LocalAgent.Controller.",
                      "param_settings": [
                        {
                          "param": "Alias",
                          "value": "test",
                          "required": true
                        },
                        {
                          "param": "EndpointID",
                          "value": "test",
                          "required": true
                        }
                      ]
                    }
                  ]
                }
              }
            }
        });
        assert_eq!(serialized_msg, expected_msg);
    }

    #[test]
    fn delete_obj() {
        let bytes: Vec<u8> = vec![
            ,
        ];
        let payload_start = 78;
        let payload_expected = &bytes[payload_start..];

        let mut record = Record::from_bytes(&bytes).expect("Invalid Record bytes");
        let serialized = serde_json::to_value(&record).unwrap();

        let expected = serde_json::json!({
            "version": "1.0",
            "to_id": "proto::ax-usp-agent-nossl-websocket",
            "from_id": "proto::ax-usp-controller-nossl",
            "payload_security": "PLAINTEXT",
            "mac_signature": [],
            "sender_cert": [],
            "no_session_context": {
                "payload": payload_expected,
            }
        });
        assert_eq!(serialized, expected);

        let payload = record.payload_flatten().unwrap();
        assert_eq!(payload_expected, payload.as_slice());

        let msg = Msg::from_bytes(&payload).expect("Invalid Msg bytes");
        let serialized_msg = serde_json::to_value(&msg).unwrap();

        let expected_msg = serde_json::json!({
            "Header": {
              "msg_id": "AXSS-1544114102.668439",
              "msg_type": "DELETE"
            },
            "Body": {
              "Request": {
                "Delete": {
                  "allow_partial": true,
                  "obj_paths": [
                    "Device.LocalAgent.MTP.1.WebSocket."
                  ]
                }
              }
            }
        });
        assert_eq!(serialized_msg, expected_msg);
    }

    #[test]
    fn get_obj() {
        let bytes: Vec<u8> = vec![
            ,
        ];
        let payload_start = 78;
        let payload_expected = &bytes[payload_start..];

        let mut record = Record::from_bytes(&bytes).expect("Invalid Record bytes");
        let serialized_record = serde_json::to_value(&record).unwrap();

        let expected = serde_json::json!({
            "version": "1.0",
            "to_id": "proto::ax-usp-agent-nossl-websocket",
            "from_id": "proto::ax-usp-controller-nossl",
            "payload_security": "PLAINTEXT",
            "mac_signature": [],
            "sender_cert": [],
            "no_session_context": {
                "payload": payload_expected,
            }
        });
        assert_eq!(serialized_record, expected);

        let payload = record.payload_flatten().unwrap();
        assert_eq!(payload_expected, payload.as_slice());

        let msg = Msg::from_bytes(&payload).expect("Invalid Msg bytes");
        let serialized_msg = serde_json::to_value(&msg).unwrap();

        let expected_msg = serde_json::json!({
              "Header": {
                "msg_id": "AXSS-1544114045.442596",
                "msg_type": "GET"
              },
              "Body": {
                "Request": {
                  "Get": {
                    "param_paths": [
                      "Device.LocalAgent.MTP.1.WebSocket."
                    ],
                    "max_depth": 0
                  }
                }
              }
        });
        assert_eq!(serialized_msg, expected_msg);
    }
}
