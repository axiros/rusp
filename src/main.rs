...
fn encode_msg_body_buf(typ: MsgType) -> Result<Vec<u8>> {
    ...
    match typ {
        ...
        MsgType::USPError { code, message } => {
            let mut error = usp_builder::ErrorBuilder::new().with_code (code);
        ...
        MsgType::USPGetResp { result } => {
            ...
            for req_path_result in getresp_json.into_iter() {
                ...
                for res_path_result in req_path_result.3.into_iter() {
                    ...
                    reqpathbuilder = reqpathbuilder.add_res_path_result (respathbuilder);
                }
                getrespb = getrespb.add_req_path_result(reqpathbuilder)
            }
        ...
        MsgType::USPNotify {
            ...
            notify = match typ {
                ...
                NotifyType::OperationComplete { obj_path, command_name, command_key, operation_resp } => match operation_resp {
                    rusp::usp_types::OperateResponse::OutputArgs(output_args) => notify.with_operation_complete_output_args(obj_path, command_name, command_key, output_args),
                    rusp::usp_types::OperateResponse::CommandFailure(err_code, err_msg) => notify.with_operation_complete_cmd_failure(obj_path, command_name, command_key, err_code, err_msg),
                }
            };
        ...
        MsgType::USPOperate {
            ...
            serialize_into_vec(&usp_builder::OperateBuilder::new(command, command_key).with_send_resp(send_resp).with_input_args(v).build()?)
        }
        MsgType::USPSet {
            allow_partial,
            args,
